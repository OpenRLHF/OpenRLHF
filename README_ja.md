...
**効率の最適化**
- SFT/DPO/PPO では互換 varlen attention backend 向けの sample packing（`--fsdp.packing_samples`）がデフォルトで有効です。packed representation は loaded model path に従い、native model は AutoModel THD、HF fallback は HF FlashAttention varlen を使います。dynamic batch は microbatch grouping だけを変え、この model-selected packing path を再利用します
...
- SDPA / eager / optional FlashAttention の選択（`--fsdp.attn_implementation`）
