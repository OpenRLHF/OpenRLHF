...
PyTorch FSDP2/DTensor をベースに、[NeMo AutoModel](https://docs.nvidia.com/nemo/automodel/) integration で model construction、TP/CP/EP mesh、activation checkpointing、LoRA/QLoRA、FSDP2 wrapping を扱います。OpenRLHF は RL loop、Ray orchestration、vLLM weight refit、checkpoint flow を薄く保ちます。
...
**モデルサポート**
- [LoRA/QLoRA](./examples/scripts/train_sft_mixtral_lora.sh)（`--fsdp.lora.rank`、`--fsdp.load_in_4bit`；RLではvLLM adapter refit未対応のため現在拒否されます）
