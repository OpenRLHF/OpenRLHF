...
OpenRLHFは、**Ray + vLLM + FSDP2(AutoTP/EP/CP)** と**統一エージェントベース設計パラダイム**を組み合わせた高性能で本番環境対応のオープンソースRLHFフレームワークです。
...
- [🗞️ ニュース](#ニュース)
- [🏗️ アーキテクチャ基盤](#アーキテクチャ基盤ray--vllm--fsdp2autotpepcp分散) - Ray + vLLM + FSDP2(AutoTP/EP/CP)分散インフラ
...
- [2026/10] [ScaleRL](https://arxiv.org/abs/2510.13786) が大規模学習シナリオにおけるREINFORCE++-baselineの有効性を検証。[REINFORCE++スライド](https://docs.google.com/presentation/d/1stieP_3PM1z4Hq1YWR3GywFkxcHEAlstXMaS23KlGN4)をリリース
- [2026/6] [Magistral](https://mistral.ai/static/research/magistral.pdf) がREINFORCE++-baselineと非常に類似した手法を用いて推論モデルを学習。
- [2026/5] [MARTI](https://github.com/TsinghuaC3I/MARTI) がOpenRLHFのフォークとしてリリース。集中型マルチエージェント相互作用と分散ポリシー学習を統合することで、LLMベースのマルチエージェントシステムをRLで学習することを目的としています。
- [2026/5] OpenRLHF 0.8.0は `--train.async_enable` による非同期RLHF学習と、`--train.agent_func_path` による非同期エージェントRLHFをサポートし、クラスベースのエージェントAPIを再設計。実行可能な例は [train_reinforce_baseline_ray_agent_async.sh](./examples/scripts/train_reinforce_baseline_ray_agent_async.sh) を参照してください。
- [2026/4] ブログ記事 [Accelerating RLHF with vLLM, Best Practice from OpenRLHF](https://blog.vllm.ai/2026/04/23/openrlhf-vllm.html) を公開
- [2026/4] Clean OpenRLHF：単一コントローラと統一パッキングサンプルに基づいてソースコードをリファクタリング
- [2026/3] CMUの[Advanced Natural Language Processing Spring 2026](https://cmu-l3.github.io/anlp-spring2026/)コースでOpenRLHFがRLHFフレームワーク教材として採用されました。
- [2026/2] [Logic-RL](https://arxiv.org/abs/2502.14768) と [PRIME](https://arxiv.org/abs/2502.01456) が、REINFORCE++がGRPOと比較してより安定し、PPOよりも高速であることを実証。
- [2026/2] [LMM-R1](https://github.com/TideDra/lmm-r1) がOpenRLHFのフォークとしてリリース。マルチモーダルタスクにおけるDeepSeek-R1の再現のための高性能RLインフラを提供することを目的としています。
- [2026/2] MITとMicrosoftがOpenRLHFを使用して [On the Emergence of Thinking in LLMs I: Searching for the Right Intuition](https://arxiv.org/pdf/2502.06773) を提案
- [2026/1] HKUSTがOpenRLHFを使用して[小規模モデルでのDeepSeek-R1-ZeroとDeepSeek-R1学習](https://github.com/hkust-nlp/simpleRL-reason)を再現
...
<a id="アーキテクチャ基盤ray--vllm--fsdp2autotpepcp分散"></a>
## 🏗️ アーキテクチャ基盤：Ray + vLLM + FSDP2(AutoTP/EP/CP)分散

OpenRLHFは、Ray + vLLM + FSDP2(AutoTP/EP/CP)分散アーキテクチャにより、学習、rollout、報酬、参照モデル、criticをGPU全体で効率的にオーケストレーションします：
...
**FSDP2(AutoTP/EP/CP) - メモリ効率的な学習**
PyTorch FSDP2/DTensor をベースに、[NeMo AutoModel](https://docs.nvidia.com/nemo/automodel/) integration で model construction、TP/CP/EP mesh、activation checkpointing、LoRA、FSDP2 wrapping を扱います。OpenRLHF は RL loop、Ray orchestration、vLLM weight refit、checkpoint flow を薄く保ちます。

**Transformers + AutoModel - モデルインターフェース**
`NeMoAutoModelForCausalLM.from_pretrained(...)` や `NeMoAutoModelForSequenceClassification.from_pretrained(...)` などの AutoModel 公式エントリ経由で HuggingFace Transformers と統合します。
...
**Ray分散アーキテクチャの上に**、OpenRLHFは**統一エージェントパラダイム**を実装しています。標準的なPPOでも複雑なマルチターン推論でも、すべての学習実行は一貫したエージェント実行パイプラインに従います。
...
| 手法 | スクリプト | 説明 |
|------|-----------|------|
| **SFT** | [train_sft.sh](./examples/scripts/train_sft.sh) | 教師あり微調整、sample packing は任意 |
...
**効率の最適化**
- SFT/DPO/PPO の sample packing（`--fsdp.packing_samples`）は任意です。actor/CausalLM の packed forward は HF fallback + `flash_attention_2` または AutoModel custom + `te` を使い、reward/critic sequence-regression packing は現在 HF fallback + `flash_attention_2` を使います
...
**スケーラビリティ**
- FSDP2 tensor parallelism（`--fsdp.tp_size`）と sequence parallelism（`--fsdp.sequence_parallel`）
- FSDP2 context parallelism（`--fsdp.cp_size`、sample packingとは同時利用不可）
- MoE expert parallelism（`--fsdp.ep_size`）
- [SLURM](./examples/scripts/train_ppo_ray_slurm.sh)を使用したマルチノード学習

**モデルサポート**
- [LoRA](./examples/scripts/train_sft_mixtral_lora.sh)（`--fsdp.lora.rank`；RLではvLLM adapter refit未対応のため現在拒否されます）
- [専門家混合（MoE）](./examples/test_scripts/train_sft_moe.sh)（`--actor.aux_loss_coef`）
- SDPA / eager / FlashAttention2 / TE attention の選択（`--fsdp.attn_implementation`）。HF fallback の `flash_attention_2` packing には `.[flash-attn-2]`、AutoModel custom の `te` packing には Transformer Engine が必要です
- HuggingFaceチャットテンプレート（`--data.apply_chat_template`）

**オプティマイザ**
- AdamW（デフォルト）：`--{actor,critic}.optim adam --{actor,critic}.adam.lr 2e-6`
- [Muon](https://kellerjordan.github.io/posts/muon/) は AutoModel の Dion-family optimizer builder 経由で使用します：`--{actor,critic}.optim muon --{actor,critic}.muon.lr 1e-4 --{actor,critic}.muon.momentum 0.95`。scalar AdamW group は外側の `--{actor,critic}.adam.*` を再利用します。Dion は base requirements に含まれます。
...
> [!TIP]
> このブランチは **NVIDIA PyTorch 26.03**、**torch 2.11**、`requirements.txt` に pin された **NeMo AutoModel main** commit、**vLLM 0.20.0** を対象にしています。Dion は PyPI ではなく `github.com/microsoft/dion` からインストールします。正確な pin は `requirements.txt` と `dockerfile/Dockerfile` を参照してください。Docker image では Dion を vLLM より先にインストールし、最終的な torch stack は vLLM/base PyTorch image に合わせます。
...
LoRAを使用する場合、OpenRLHFはアダプターの重みのみを保存します。デプロイまたは学習を続けるには、アダプターをベースモデルとマージします：
...
私たちのプロジェクトは[ColossalChat](https://github.com/hpcaitech/ColossalAI/tree/main/applications/ColossalChat)と[DeepSpeedChat](https://github.com/microsoft/DeepSpeedExamples/tree/master/applications/DeepSpeed-Chat)にも感謝します。プロジェクトの初期段階で、彼らのコード設計を参考にしました。また、初期の ring-attention 探索で GPU サポートを提供してくれた [Netmind.AI](https://www.netmind.ai/) に感謝します。
