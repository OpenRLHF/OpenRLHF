...
OpenRLHFは、**Ray + vLLM + NVIDIA NeMo AutoModel/FSDP2** と**統一エージェントベース設計パラダイム**を組み合わせた**最初の**高性能で本番環境対応のオープンソースRLHFフレームワークです。
...
- [🗞️ ニュース](#ニュース)
- [🏗️ アーキテクチャ基盤](#アーキテクチャ基盤ray--vllm--automodelfsdp2分散) - Ray + vLLM + AutoModel/FSDP2分散インフラ
...
<a id="アーキテクチャ基盤ray--vllm--automodelfsdp2分散"></a>
## 🏗️ アーキテクチャ基盤：Ray + vLLM + AutoModel/FSDP2分散

OpenRLHFは、Ray + vLLM + AutoModel/FSDP2分散アーキテクチャをベースに構築された**最初のRLHFフレームワーク**であり、GPU全体で複数のコンポーネントを効率的にオーケストレーションします：
...
**NeMo AutoModel + PyTorch FSDP2 - メモリ効率的な学習**
[NVIDIA NeMo AutoModel](https://docs.nvidia.com/nemo/automodel/) と PyTorch FSDP2/DTensor をベースに構築されています。AutoModel がモデル構築、TP/CP/EP mesh、activation checkpointing、LoRA/QLoRA、FSDP2 wrapping を担当し、OpenRLHF は RL ループ、Ray orchestration、vLLM weight refit、checkpoint flow を薄く保ちます。
...
**効率の最適化**
- 互換 varlen attention backend 向けの optional sample packing（`--fsdp.packing_samples`）
...
**スケーラビリティ**
- AutoModel tensor parallelism（`--fsdp.tp_size`）と sequence parallelism（`--fsdp.sequence_parallel`）
- AutoModel context parallelism（`--fsdp.cp_size`、sample packingとは同時利用不可）
...
- [Muon](https://kellerjordan.github.io/posts/muon/) は AutoModel の Dion-family optimizer builder 経由で使用します：`--{actor,critic}.optim muon --{actor,critic}.muon.lr 1e-4 --{actor,critic}.muon.momentum 0.95`。scalar AdamW group は外側の `--{actor,critic}.adam.*` を再利用します。Dion は `pip install "openrlhf[dion]"` または Docker image でインストールしてください。
...
> [!TIP]
> このブランチは **NVIDIA PyTorch 25.11**、**torch 2.11**、**NeMo AutoModel main commit `17ed5796bdc220c314c9fd6bd718a773a3642521`**、**vLLM 0.20.0** を対象にしています。Dion は PyPI ではなく `github.com/microsoft/dion` からインストールします。
