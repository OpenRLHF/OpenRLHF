...
from openrlhf.models.loss import DisCOBasicLoss, DisCOLoss, DisCOHelper
...
@pytest.mark.parametrize("kl_val, delta, beta, expected_penalty_val", [
...
def test_disco_helper_calculate_kl_penalty(kl_val, delta, beta, expected_penalty_val):
    ...
    penalty_no_mask, kl_div_detached_no_mask = DisCOHelper.calculate_kl_penalty(log_probs, old_log_probs, beta, delta, action_mask=None)
...
def test_disco_basic_loss_mixed_rewards():
    ...
    old_log_probs_for_test = _t([[0.12, 0.22], [0.32, 0.42], [0.52, 0.12], [0.22, 0.22]]) # sums: 0.34, 0.74, 0.64, 0.44
    ...
    actual_loss_logl, actual_kl_logl, actual_j1_logl, actual_penalty_logl = loss_fn_logl(
        log_probs_for_test, old_log_probs_for_test, rewards_binary, action_mask=None # action_mask=None means full sequence sum for score
    )
...
def test_disco_loss_mixed_rewards():
    ...
    N_neg = torch.tensor(s_negative.numel(), dtype=tau.dtype if isinstance(tau,torch.Tensor) else torch.float, device=DEVICE)
