...
class GRPOTrainer(ABC):
    ...
    Args:
        ...
        train_dataloader (DataLoader): The dataloader for training dataset.
        eval_dataloader (DataLoader): The dataloader for evaluation dataset.
        ...
        beta (float, defaults to 0.1): Reward scaling coefficient.
        gamma (float, defaults to 0.1): KL penalty coefficient.
        temperature (float, defaults to 1.0): Temperature for policy exploration.
    ...
    def compute_grpo_loss(
        ...
        Returns:
            ...
            - kl_div: The KL divergence between current and reference policy
        """
    ...
    def concatenated_forward(self, model, chosen_ids, c_mask, reject_ids, r_mask, prompt_id_lens):
        ...
        Similar to DPO but with temperature scaling for exploration.
        """
        ...
        all_logits = output["logits"] / self.temperature  # Apply temperature scaling
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            for data in self.train_dataloader:
                if not self.packing_samples:
                    ...
                    # Get logprobs from current policy and reference model
                    ...
                    # Compute GRPO loss
                    loss, rewards, kl_div = self.compute_grpo_loss(
                ...
                loss = loss + aux_loss * self.args.aux_loss_coef + nll_loss * self.args.nll_loss_coef
                self.strategy.backward(loss, self.model, self.optimizer)
                self.strategy.optimizer_step(self.optimizer, self.model, self.scheduler)
