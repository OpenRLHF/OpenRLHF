...
class UnpairedRewardDataset(Dataset):
    ...
    def __init__(self, dataset, tokenizer: Callable = None, 
                 max_length: int = None, strategy = None, vanilla_loss = False) -> None:
        super().__init__()
    ...
    def collate_fn(self, item_list):
        ...
        if not self.vanilla_loss:
            for prompt_idx in range(len(item_list)):
                ...
                input_ids, attention_mask = concat_to_tensor(item_list[prompt_idx][0], 
                                                             item_list[response_idx][1])
                response_ids.append(input_ids)
                response_masks.append(attention_mask)
                labels.append(-1)
...
class DistributedVanillaKTOSampler(DistributedSampler):
    ...
    def __iter__(self):
        ...
        assert len(self.label_to_indices[0]) == len(self.label_to_indices[1]), \
