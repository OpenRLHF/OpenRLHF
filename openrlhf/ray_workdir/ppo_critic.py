...
class CriticModelRayActor(BasePPORole):
    ...
    def compute_value(self, input_ids, attention_mask=None):
        device = torch.cuda.current_device()
        self.critic.eval()
        with torch.no_grad():
            value = self.critic.compute_value(input_ids.to(device), attention_mask.to(device), return_dict=False)[0].cpu()
