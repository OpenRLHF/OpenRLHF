...
def get_llm_for_sequence_classification(
    model_name_or_path, config=None, num_labels=1, use_flash_attention_2=False, **kwargs
):
    """Get transformer with a sequence classification head on top (linear layer).

    Args:
        model_name_or_path (_type_): _description_
    ...
    try:
        ...
        cls_class = _get_model(base_pretrained_class, base_class)
...
def _get_model(base_pretrained_model, base_llm_model):
    class LLMForSequenceClassification(base_pretrained_model):
        ...
        def forward(
            ...
            if action_mask:
                return self._forward_critic(last_hidden_states, action_mask)
            else:
                return self._forward_reward(last_hidden_states, attention_mask)

        def _forward_critic(self, last_hidden_states, action_mask):
        ...
        def _forward_reward(self, last_hidden_states, attention_mask):
