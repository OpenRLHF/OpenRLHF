...
class VanillaKTOLoss(nn.Module):
    ...
    def forward(
        ...
        losses = torch.cat((1 - F.sigmoid(self.beta * (chosen_logratios - rejected_KL)), 1 - F.sigmoid(self.beta * (chosen_KL - rejected_logratios))), 0).mean()
...
class KTOLoss(nn.Module):
    ...
    def forward(
        ...
        losses = torch.cat((self.desirable_weight * chosen_losses, 
                            self.undesirable_weight * rejected_losses), 0).mean()
