...
from openrlhf.utils.fsdp2.tp.loss_parallel import _unpack_sharded_logits
...
def _worker_unpack_sharded_logits(rank: int, world_size: int, port: int) -> None:
    _init_process_group(rank, world_size, port)
    try:
        mesh = _make_device_mesh(world_size)
        full_logits = _build_reference_full_logits()
        ...
        assert local_vocab == LOCAL_VOCAB_SIZES[rank]
        assert global_vocab == GLOBAL_VOCAB_SIZE
        assert vocab_start == expected_start
        assert vocab_end == expected_end
        assert_close(unpacked_local, local_logits, rtol=RTOL, atol=ATOL)
    finally:
        dist.destroy_process_group()


@pytest.mark.unit
def test_unpack_sharded_logits() -> None:
    _run_distributed_test(_worker_unpack_sharded_logits)
