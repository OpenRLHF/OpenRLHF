...
def main():
    parser = argparse.ArgumentParser(
        description="Compare loss values between DeepSpeed and FSDP2 backends"
    )
    parser.add_argument(
        "--deepspeed_log",
        type=str,
        default="logs/deepspeed_test.log",
        help="Path to DeepSpeed log file"
    )
    parser.add_argument(
        "--fsdp2_log",
        type=str,
        default="logs/fsdp2_test.log",
        help="Path to FSDP2 log file"
    )
    parser.add_argument(
        "--threshold",
        type=float,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional path to save comparison plot"
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
        help="Print all loss values"
    )
    ...
    # Compare backends
    aligned, max_diff, message = compare_loss_curves(
        ds_losses, fsdp2_losses, threshold=args.threshold
    )
