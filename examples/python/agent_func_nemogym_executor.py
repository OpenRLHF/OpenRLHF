...
def prepare_data(question: str, expected_answer: str, sampling_params: SamplingParams) -> Dict[str, Any]:
    ...
    agent_ref = {
        ...
        "name": "library_judge_math_simple_agent"
    }
...
class AgentExecutor(AgentExecutorBase):
        ...
        for i in range(max_retries):
            try:
                response = requests.get(
                    f"http://{self.vllm_server_host}:{self.vllm_server_port}/health",
                    timeout=5
                )
    ...
    def _start_vllm_server(self, llm_engine, hf_tokenizer):
        ...
        async def health():
            ...
            return {
                "status": "healthy",
                "model": getattr(hf_tokenizer, "name_or_path", "unknown")
            }
                ...
                "choices": [
                    {
                        ...
                        "finish_reason": "stop",
                        "logprobs": {
                            "content": generated_logprobs_list
                        }
                    }
                ],
                "usage": {
