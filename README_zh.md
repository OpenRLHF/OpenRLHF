...
OpenRLHF 是结合 **Ray + vLLM + FSDP2(AutoTP/EP/CP)** 与**统一 Agent 设计范式**的高性能、生产就绪的开源 RLHF 框架，用于可扩展的人类反馈强化学习。
...
- [🗞️ 新闻](#新闻)
- [🏗️ 架构基础](#架构基础ray--vllm--fsdp2autotpepcp-分布式) - Ray + vLLM + FSDP2(AutoTP/EP/CP) 分布式基础设施
...
<a id="架构基础ray--vllm--fsdp2autotpepcp-分布式"></a>
## 🏗️ 架构基础：Ray + vLLM + FSDP2(AutoTP/EP/CP) 分布式

OpenRLHF 基于 Ray + vLLM + FSDP2(AutoTP/EP/CP) 分布式架构，可高效地跨 GPU 编排训练、rollout、奖励、参考模型和 critic 等组件：
...
**FSDP2(AutoTP/EP/CP) - 内存高效训练**
基于 PyTorch FSDP2/DTensor，并集成 [NeMo AutoModel](https://docs.nvidia.com/nemo/automodel/) 处理模型构建、TP/CP/EP mesh、activation checkpointing、LoRA 与 FSDP2 包装；OpenRLHF 保持 RL 训练循环、Ray 编排、vLLM 权重同步和 checkpoint 流程轻量。

**Transformers + AutoModel - 模型接口**
通过 `NeMoAutoModelForCausalLM.from_pretrained(...)`、`NeMoAutoModelForSequenceClassification.from_pretrained(...)` 等 AutoModel 官方入口与 HuggingFace Transformers 集成。
...
**在 Ray 分布式架构之上**，OpenRLHF 实现了**统一 Agent 范式**。无论是标准 PPO 还是复杂的多轮推理，每次训练运行都遵循一致的 Agent 执行流程。
...
**可扩展性**
- FSDP2 张量并行（`--fsdp.tp_size`）和 sequence parallel（`--fsdp.sequence_parallel`）
- FSDP2 长上下文 CP（`--fsdp.cp_size`，当前不能与 sample packing 同时使用）
- MoE 专家并行（`--fsdp.ep_size`）
...
- [LoRA](./examples/scripts/train_sft_mixtral_lora.sh)（`--fsdp.lora.rank`；RL 当前会拒绝 LoRA，因为 vLLM adapter refit 尚未接入）
...
- [Muon](https://kellerjordan.github.io/posts/muon/) 通过 AutoModel 的 Dion-family optimizer builder：`--{actor,critic}.optim muon --{actor,critic}.muon.lr 1e-4 --{actor,critic}.muon.momentum 0.95`。标量 AdamW 参数复用外层 `--{actor,critic}.adam.*`。Dion 已放入基础 requirements。
...
# 或者：bash examples/scripts/docker_run.sh
...
OpenRLHF 的模型检查点与 HuggingFace 模型兼容。SFT/RM/DPO 使用 `--model.model_name_or_path {name_or_path}`；RL 使用 `--actor.model_name_or_path`、`--reward.model_name_or_path` 和 `--critic.model_name_or_path`。我们在 [HuggingFace OpenRLHF](https://huggingface.co/OpenRLHF) 上提供了一些预训练检查点和数据集。
...
```bash
torchrun --standalone --nproc_per_node=8 -m openrlhf.cli.train_sft \
   --data.max_len 4096 \
...
# --fsdp.tp_size 2                           # 启用 FSDP2 tensor parallel
# --fsdp.cp_size 2                           # 启用 FSDP2 context parallel（需关闭 packing）
...
```bash
torchrun --standalone --nproc_per_node=8 -m openrlhf.cli.train_rm \
...
Reward/Critic 默认使用 `--fsdp.value_head_prefix score`。训练后的 checkpoint 仍可通过 HuggingFace sequence-classification 方式加载：
...
> [!NOTE]
> **Ray 环境设置**：生产环境建议每个节点预先构建 Docker 镜像，也可以让 Ray 使用 `--runtime-env-json='{"working_dir": "/openrlhf", "setup_commands": ["pip install -e \"/openrlhf[vllm]\""]}'` 安装源码树。

> [!NOTE]
> **GPU 索引错误故障排除**：如果 Ray 对 FSDP worker 隐藏 GPU，请设置 `export RAY_EXPERIMENTAL_NOSET_CUDA_VISIBLE_DEVICES=1`。
...
使用 LoRA 时，OpenRLHF 仅保存适配器权重。要部署或继续训练，请将适配器与基础模型合并：
...
| 模式 | 相关参数 | 特性 | 适用场景 |
|------|---------|------|---------|
| **混合引擎（colocated）** | `--train.colocate_all`<br>`--vllm.enable_sleep`<br>`--fsdp.enable_sleep` | **最稳定** ——严格 on-policy，每次 rollout 使用最新权重，生成→训练串行执行 | 研究、对 off-policy 敏感的 RL 算法、复现、配方验证 |
...
**当您有足够内存时**：
- ✅ 保持 `--fsdp.cpu_offload` 关闭
- ✅ 先验证非 colocated 稳定后，再启用 actor/ref 或 critic/reward colocate
