...
OpenRLHF 是**首个**结合 **Ray + vLLM + NVIDIA NeMo AutoModel/FSDP2** 与**统一 Agent 设计范式**的高性能、生产就绪的开源 RLHF 框架，用于可扩展的人类反馈强化学习。
...
- [🗞️ 新闻](#新闻)
- [🏗️ 架构基础](#架构基础ray--vllm--automodelfsdp2-分布式) - Ray + vLLM + AutoModel/FSDP2 分布式基础设施
...
<a id="架构基础ray--vllm--automodelfsdp2-分布式"></a>
## 🏗️ 架构基础：Ray + vLLM + AutoModel/FSDP2 分布式

OpenRLHF 是**首个**基于 Ray + vLLM + AutoModel/FSDP2 分布式架构构建的 RLHF 框架，可高效地跨 GPU 编排多个组件：
...
**NeMo AutoModel + PyTorch FSDP2 - 内存高效训练**
基于 [NVIDIA NeMo AutoModel](https://docs.nvidia.com/nemo/automodel/) 与 PyTorch FSDP2/DTensor。AutoModel 负责模型构建、TP/CP/EP mesh、activation checkpointing、LoRA/QLoRA 与 FSDP2 包装；OpenRLHF 保持 RL 训练循环、Ray 编排、vLLM 权重同步和 checkpoint 流程轻量。
...
**可扩展性**
- AutoModel 张量并行（`--fsdp.tp_size`）和 sequence parallel（`--fsdp.sequence_parallel`）
- AutoModel 长上下文 CP（`--fsdp.cp_size`，当前不能与 sample packing 同时使用）
...
- [Muon](https://kellerjordan.github.io/posts/muon/) 通过 AutoModel 的 Dion-family optimizer builder：`--{actor,critic}.optim muon --{actor,critic}.muon.lr 1e-4 --{actor,critic}.muon.momentum 0.95`。标量 AdamW 参数复用外层 `--{actor,critic}.adam.*`。Dion 需通过 `pip install "openrlhf[dion]"` 或 Docker 镜像安装。
...
# --fsdp.tp_size 2                           # 启用 AutoModel tensor parallel
# --fsdp.cp_size 2                           # 启用 AutoModel context parallel（需关闭 packing）
...
> [!NOTE]
> **GPU 索引错误故障排除**：如果 Ray 对 AutoModel/FSDP worker 隐藏 GPU，请设置 `export RAY_EXPERIMENTAL_NOSET_CUDA_VISIBLE_DEVICES=1`。
